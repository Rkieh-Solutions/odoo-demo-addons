...
def diagnose():
    ...
    try:
        ...
        # 2. Check for ANY non-clean module states
        ...
        # 3. Check Menus (Handling JSONB)
        print("\n[All Menus that might be PHAR]")
        # In Odoo 19/20, 'name' is often a JSONB for translations
        # Let's try to get the ID and Name cast to text
        cur.execute("SELECT id, name::text, parent_id FROM ir_ui_menu WHERE name::text ILIKE '%phar%' OR name::text ILIKE '%pharmacy%';")
        menus = cur.fetchall()
        for menu in menus:
        ...
        if mcount > 0:
            cur.execute("SELECT name, model, res_id FROM ir_model_data WHERE module = 'phar' LIMIT 20;")
            rows = cur.fetchall()
            for row in rows:
                print(f"  Name: {row[0]} | Model: {row[1]} | ID: {row[2]}")
                
        # 5. Check ir_model_data for 'pharmacy' module
